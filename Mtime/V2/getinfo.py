...
class pageinfo():
    ...
    def getMovieContent(self,x, page):
        ...
        movieDIR['MovieName'] = MovieName = str(html.unescape(re.findall(MovieNameReg, page)[0])).replace("'", "`")
        # 2影片别名
        try:
            MovieNameOtherReg = '<p class="db_enname" style="font-size:25px;">(.*?)</p>'
            movieDIR['MovieNameOther'] = MovieNameOther = re.findall(MovieNameOtherReg, page, re.S)[0].replace("'", "`")
        except:
            movieDIR['MovieNameOther'] = MovieNameOther = '-'

        # 3影片海报
        MoviePostReg = '<img src="(.*?)" alt=".*?" rel="v:image" />'
        try:
            movieDIR['MoviePost'] = MoviePost = re.findall(MoviePostReg, page)[0]
        except:
            movieDIR['MoviePost'] =  MoviePost = '-'

        # 4影片类型(多个)
        MovieTypeReg = 'property="v:genre">(.*?)</a>'
        movieDIR['MovieType'] = MovieType = str(re.findall(MovieTypeReg, page)).replace('[', '').replace(']', '').replace('\'', '').replace(',', '/').replace(' ', '')
        ...
        try:
            movieDIR['ReleasedDate'] = ReleasedDate = re.findall(ReleasedDateReg, page)[0]
        except:
            movieDIR['ReleasedDate'] = ReleasedDate = '-'

        # 6上映地区
        ReleasedAreaReg = 'property="v:initialReleaseDate" content=".*?">.*?</a>(.*?)上映</div>'
        try:
            movieDIR['ReleasedArea'] = ReleasedArea = re.findall(ReleasedAreaReg, page, re.S)[0]
        except:
            movieDIR['ReleasedArea'] = ReleasedArea = '-'
        ...
        try:
            movieDIR['MovieNation'] = MovieNation = html.unescape(re.findall(MovieNationReg, page, re.M)[0])
        except:
            movieDIR['MovieNation'] = MovieNation = '-'

        # 8导演
        try:
            MovieDirectorReg = ' rel="v:directedBy">(.*?)</a>'
            movieDIR['MovieDirector'] = MovieDirector = html.unescape(
                str(re.findall(MovieDirectorReg, page)).replace('[', '').replace(']', '').replace('\'', '').replace(' ', '')).replace(',', '，')
        except:
            movieDIR['MovieDirector'] = MovieDirector = '-'
        # 9编剧 人员
        try:
            ScreenWriterReg = '  <a href="http://people.mtime.com/.*?/" target="_blank">(.*?)</a>'
            movieDIR['ScreenWriter'] = ScreenWriter = html.unescape(
                str(re.findall(ScreenWriterReg, page)).replace('[', '').replace(']', '').replace('\'', '').replace(' ', '').replace(',', '，'))
        except:
            movieDIR['ScreenWriter'] = ScreenWriter = '-'

        # 10发行公司
        DistributionEnterpriseReg = '<a href="http://movie.mtime.com/company/.*?/" target="_blank">(.*?)</a>'
        try:
            movieDIR['DistributionEnterp'] = DistributionEnterprise = str(html.unescape(re.findall(DistributionEnterpriseReg, page)[0]))
        except:
            movieDIR['DistributionEnterp'] = DistributionEnterprise = '-'

        # 11剧情介绍(要求，去除段首的空格)
        MoviePlotReg = '<p class="mt6 lh18">(.*?)</p>'
        try:
            movieDIR['MoviePlot'] = MoviePlot = str(html.unescape(
                str(re.findall(MoviePlotReg, page, re.S)[0]).replace('\r\n', '').replace('　', '').replace(' ', '')))
        except:
            movieDIR['MoviePlot'] = MoviePlot = '-'

        # 12上映年份
        ReleasedYearReg = '<a  style="font-size:30px;" href=".*?" target="_blank">(.*?)</a>'
        try:
            movieDIR['ReleasedYear'] = ReleasedYear = str(re.findall(ReleasedYearReg, page)[0])
        except:
            movieDIR['ReleasedYear'] = ReleasedYear = '-'
        # ----------可以套用方法了

        # 13影片视频数量
        TrailerNumReg = '<span>(.*?)</span> 个视频</a>'
        TrailerUrlUrl = 'href="(.*?)"><span>.*?</span> 个视频</a><i>&nbsp;</i></dd>'
        movieDIR['TrailerNum'] = TrailerNum = str(self.getNumUrl(pageinfo,page, TrailerNumReg, TrailerUrlUrl)[0])

        # 14影片视频详情url
        movieDIR['TrailerUrl'] = TrailerUrl = str(self.getNumUrl(pageinfo,page, TrailerNumReg, TrailerUrlUrl)[1])

        # 15演员数量
        ActorNumReg = '<span>(.*?)</span> 位演职员</a><i>&nbsp;</i></dd>'
        ActorUrlReg = '<a href="(.*?)"><span>.*?</span> 位演职员</a><i>&nbsp;</i></dd>'
        movieDIR['ActorNum'] = ActorNum = str(self.getNumUrl(pageinfo,page, ActorNumReg, ActorUrlReg)[0])

        # 16演员详情网页
        movieDIR['ActorUrl'] = ActorUrl = str(self.getNumUrl(pageinfo,page, ActorNumReg, ActorUrlReg)[1])

        # 17图片数量
        ImageNumReg = '<span>(.*?)</span> 张图片</a><i>&nbsp;</i></dd>'
        ImageUrlReg = '<a href="(.*?)"><span>.*?</span> 张图片</a><i>&nbsp;</i></dd>'
        movieDIR['ImageNum'] = ImageNum = str(self.getNumUrl(pageinfo,page, ImageNumReg, ImageUrlReg)[0])

        # 18图片详情网页
        movieDIR['ImageUrl'] = ImageUrl = str(self.getNumUrl(pageinfo,page, ImageNumReg, ImageUrlReg)[1])
        ...
        movieDIR['CommentNum'] = CommentNum = str(self.getNumUrl(pageinfo,page, CommentNumReg, CommentUrlReg)[0])

        # 20长影评网页
        movieDIR['CommentUrl'] = CommentUrl = str(self.getNumUrl(pageinfo,page, CommentNumReg, CommentUrlReg)[1])

        # 21短影评网页
        movieDIR['CommentShortUrl'] = CommentShortUrl = Url + 'reviews/short/new.html'

        # 22影片新闻数量
        NewsUrlNumReg = '<span>(.*?)</span> 条新闻</a><i>&nbsp;</i></dd>'
        NewsUrlReg = '<a href="(.*?)"><span>.*?</span> 条新闻</a><i>&nbsp;</i></dd>'
        movieDIR['NewsNum'] = NewsNum = str(self.getNumUrl(pageinfo,page, NewsUrlNumReg, NewsUrlReg)[0])
        # 22影片新闻数量
        movieDIR['NewsUrl'] = NewsUrl = str(self.getNumUrl(pageinfo,page, NewsUrlNumReg, NewsUrlReg)[1])
...
# 获取影片评分信息，传入参数为Mtime的movie_id，返回值为数据库的列明和值
    def getRatings(self, x):
        ...
        try:
            ...
            # =======================↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓以下为数据信息↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓=============================
            movieRATE['movie_id'] = movie_Id = str(x)
            # 评级信息内容-影片评分
            try:
                movieRATE['RatingFinal'] = RatingFinal = str(movieRating['RatingFinal'])
            except:
                movieRATE['RatingFinal'] = RatingFinal = '-'

            # 评级信息内容-参与评分人数
            try:
                movieRATE['Usercount'] = Usercount = str(movieRating['Usercount'])
            except:
                movieRATE['Usercount'] = Usercount = '-'
            # 评级信息内容-想看影片人数
            try:
                movieRATE['AttitudeCount'] = AttitudeCount = str(movieRating['AttitudeCount'])
            except:
                movieRATE['AttitudeCount'] = AttitudeCount = '-'
            # =======================↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑以上为数据信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑=============================
            # 影片票房信息
            ...
            # 影片票房信息-当前影片总票房排名
            try:
                movieRATE['Rank'] = Rank = str(boxOffice['Rank'])
            except:
                movieRATE['Rank'] = Rank = '-'
            # 影片票房信息-当前票房总计
            try:
                movieRATE['TotalBoxOffice'] = TotalBoxOffice = str(boxOffice['TotalBoxOffice']) + str(boxOffice['TotalBoxOfficeUnit'])
            except:
                movieRATE['TotalBoxOffice'] = TotalBoxOffice = '-'
            # 影片票房信息-实时票房总计
            try:
                movieRATE['TodayBoxOffice'] = TodayBoxOffice = str(boxOffice['TodayBoxOffice']) + str(boxOffice['TodayBoxOfficeUnit'])
            except:
                movieRATE['TodayBoxOffice'] = TodayBoxOffice = '-'
            # 影片票房信息-上映天数
            try:
                movieRATE['ShowDays'] = ShowDays = str(boxOffice['ShowDays'])
            except:
                movieRATE['ShowDays'] = ShowDays = '-'
            # 影片票房信息-统计时间(每五分钟更新)
            try:
                movieRATE['EndDate'] = EndDate = str(boxOffice['EndDate'])
            except:
                movieRATE['EndDate'] = EndDate = '-'
            # 影片票房信息-上映首日票房
            try:
                movieRATE['FirstDayBoxOffice'] = FirstDayBoxOffice = str(boxOffice['FirstDayBoxOffice']) + str(boxOffice['FirstDayBoxOfficeUnit'])
            except:
                movieRATE['FirstDayBoxOffice'] = FirstDayBoxOffice = '-'
            # 上映情况
            try:
                movieRATE['releaseType'] = releaseType = str(result['releaseType'])
            except:
                movieRATE['releaseType'] = releaseType = '-'
            # =======================↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑以上为数据信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑=============================
            # 输出信息
...
# 合并字段及值
    def addContent(self,value1, value2):
        values = []
